...
func generateBootstrapFile(xdsServerUri string, vpcNetworkName string, outputFilePath string) error {
	...
	// Retrieve zone from the metadata trafficDirectorServer only if not specified in args.
	zone, err := getZone()
	if err != nil {
		return fmt.Errorf("could not discover GCP zone: %w", err)
	}

	// Generate deployment info from metadata trafficDirectorServer or from command-line
...
func generate(in configInput) ([]byte, error) {
	c := &config{
		XdsServers: []trafficDirectorServer{
			{
	...
	if in.includePSMSecurity {
		...
		c.ServerListenerResourceNameTemplate = "grpc/trafficDirectorServer?xds.resource.listening_address=%s"
	}
...
func getZone() (string, error) {
	...
	if i == -1 {
		return "", fmt.Errorf("could not parse zone from metadata trafficDirectorServer: %s", qualifiedZone)
	}
...
func getProjectId() (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("could not parse project id from metadata trafficDirectorServer: %w", err)
	}
...
func getFromMetadata(urlStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed communicating with metadata trafficDirectorServer: %w", err)
	}
	body, err := ioutil.ReadAll(resp.Body)
	resp.Body.Close()
	if err != nil {
		return nil, fmt.Errorf("failed reading from metadata trafficDirectorServer: %w", err)
	}
	return body, nil
}
...
type config struct {
	XdsServers                         []trafficDirectorServer              `json:"xds_servers,omitempty"`
